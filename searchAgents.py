...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        visited_corners = {self.corners[0]: False, self.corners[1]: False,
                           self.corners[2]: False, self.corners[3]: False}
        state = (self.startingPosition, visited_corners)
        return state
    ...
    def expand(self, state):
        ...
        children = []
        for action in self.getActions(state):
            child_state = self.getNextState(state, action)
            children.append((child_state, action, self.getActionCost(state, action, child_state)))
        self._expanded += 1 # DO NOT CHANGE
        return children
    ...
    def getNextState(self, state, action):
        ...
        state1_copy = state[1].copy()
        if new_state in state1_copy.keys():
            state1_copy[new_state] = True
            
        # you will need to replace the None part of the following tuple.
        return ((nextx, nexty), state1_copy)
...
def cornersHeuristic(state, problem):
    ...
    #if we're at the goal state (all 4 corners reached), return 0
    if all(value == True for value in state[1].values()):
        return 0

    #we only want to visit corners we have not yet, aka don't revisit corners
    closed_corners = []
    for corner in corners:
        if state[1][corner]:
            closed_corners.append(corner)

    maxdistance = 0
    
    for i, corner in enumerate(corners):
        if corners[i] in closed_corners:
            continue
        heuristic = util.manhattanDistance(state[0], corners[i])
        if heuristic > maxdistance:
            maxdistance = heuristic 
        closed_corners.append(corners[i])
    return maxdistance
