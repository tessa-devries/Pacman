...
def sentence2():
    ...
    D = Expr("D")
    clause_1 = C % (B | D)
    clause_2 = A >> conjoin(~B, ~D)
    clause_3 = ~conjoin(B, ~C) >> A
    clause_4 = ~D >> C
...
def sentence3():
    ...
    Pacman_Alive_1 = PropSymbolExpr('PacmanAlive', 1)
    Pacman_Alive_0 = PropSymbolExpr('PacmanAlive', 0)
    Pacman_Born_0 = PropSymbolExpr('PacmanBorn', 0)
    Pacman_Killed_0 = PropSymbolExpr('PacmanKilled', 0)
    clause_1 = Pacman_Alive_1 % disjoin(conjoin(Pacman_Alive_0, ~Pacman_Killed_0),
                                        conjoin(~Pacman_Alive_0, Pacman_Born_0))
    clause_2 = ~conjoin(Pacman_Alive_0, Pacman_Born_0)
    clause_3 = Pacman_Born_0
    return conjoin(clause_1, clause_2, clause_3)
...
def findModel(sentence):
    ...
    cnf_form = to_cnf(sentence)
    return pycoSAT(cnf_form)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals):
    ...
    for combo in combinations:
        final_cnf.append(disjoin(~combo[0], ~combo[1]))
    return conjoin(final_cnf)
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals):
    ...
    the expressions in the list is true.c
    """
    "*** BEGIN YOUR CODE HERE ***"
...
def pacphysics_axioms(t, all_coords, non_outer_wall_coords):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    sentence_1 = conjoin([(PropSymbolExpr(wall_str, coord[0], coord[1]) >> ~PropSymbolExpr(pacman_str, coord[0], coord[1], t)) for coord in all_coords])
    sentence_2 = exactlyOne([PropSymbolExpr(pacman_str, coord[0], coord[1], t) for coord in non_outer_wall_coords])
    sentence_3 = exactlyOne([PropSymbolExpr(action_str, t) for action_str in DIRECTIONS])
    
    pacphysics_sentences = [sentence_1, sentence_2, sentence_3]
