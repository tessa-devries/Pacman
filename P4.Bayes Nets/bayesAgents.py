...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    varElimination = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    poss_assignments = varElimination.getAllPossibleAssignmentDicts()
    best_assignment = dict()
    probability = 0

    for assignment in poss_assignments:
        assignmentProb = varElimination.getProbability(assignment)
        if assignmentProb > probability:
            probability = assignmentProb
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        global_variable = [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
        factor = inference.inferenceByVariableElimination(self.bayesNet, global_variable, evidence, eliminationOrder)

        
        left = {FOOD_HOUSE_VAR: TOP_LEFT_VAL, GHOST_HOUSE_VAR: TOP_RIGHT_VAL}
        right = {FOOD_HOUSE_VAR: TOP_RIGHT_VAL, GHOST_HOUSE_VAR: TOP_LEFT_VAL}
        
        left_updated = evidence.copy()
        left_updated[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        left_updated[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL
    
        right_updated = evidence.copy()
        right_updated[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        right_updated[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        
        left_prob = factor.getProbability(left_updated)
        right_prob = factor.getProbability(right_updated)
        
        leftExpectedValue = WON_GAME_REWARD * left_prob + GHOST_COLLISION_REWARD * right_prob
        rightExpectedValue = GHOST_COLLISION_REWARD * left_prob +  WON_GAME_REWARD * right_prob
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        explorationEvidence = self.getExplorationProbsAndOutcomes(evidence)
        for evidence in explorationEvidence:
            values = self.computeEnterValues(evidence[1], enterEliminationOrder)
            max_v = max(values)
            expectedValue += evidence[0] * max_v
        "*** END YOUR CODE HERE ***"
