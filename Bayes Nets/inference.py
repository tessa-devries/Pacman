...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for eliminationVar in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, eliminationVar)
            if len(joinedFactor.unconditionedVariables()) > 1:
                currentFactorsList.append(eliminate(joinedFactor, eliminationVar))
        full_joint = joinFactors(currentFactorsList)
        normalized = normalize(full_joint)
        return normalized
        "*** END YOUR CODE HERE ***"
